...
class RestrictedA1(Prior):
    def __init__(self, q=1, xeff=0):
        self.q = q
        self.xeff = xeff
        super().__init__(
            name='a1', latex_label=r'$a_1$',
            minimum=0, maximum=1, unit=None,
            boundary=None)

    def rescale(self, val):
        """
        'Rescale' a sample from the unit line element to the A1 prior.
    ...
    def prob(self, val):
        ...
        return get_p_a1_given_xeff_q(a1=val, q=self.q, xeff=self.xeff)


class RestrictedCos1(Prior):
    def __init__(self, q=1, xeff=0, a1=0):
        self.q = q
        self.xeff = xeff
        self.a1 = a1
        super().__init__(
            name='cos_tilt_1', latex_label=r'$cos\theta_1$',
            minimum=-1, maximum=1, unit=None,
            boundary=None)
    ...
    def prob(self, val):
        """Return the prior probability of val"""
        return get_p_cos1_given_xeff_q_a1(cos1=val, a1=self.a1, q=self.q, xeff=self.xeff)


class RestrictedCos2(Prior):
    def __init__(self, q=1, xeff=0, a1=0, cos1=0):
        self.q = q
        self.xeff = xeff
        self.a1 = a1
        self.cos1 = cos1
    ...
    def prob(self, val):
        """Return the prior probability of val"""
        return get_p_cos2_given_xeff_q_a1_cos1(cos2=val, cos1=self.cos1, a1=self.a1, q=self.q, xeff=self.xeff)


class RestrictedPrior:
    def __init__(self, q=1, xeff=0, clean=False, build_cache=True):
        self.q = q
        self.xeff = xeff
    ...
    def get_a1_prior(self):
        ...
        if os.path.exists(fname):
            ...
            p_a1 = np.array([get_p_a1_given_xeff_q(a1=a1, xeff=self.xeff, q=self.q) for a1 in a1s])
        ...
        return Interped(xx=a1, yy=p_a1, minimum=min(a1), maximum=max(a1), name="a_1", latex_label=r"$a_1$")

    def get_cos1_prior(self, given_a1, n=int(5e4)):
        fname = os.path.join(self.outdir, "cos1_given_qxeffa1.h5")
        if os.path.isfile(fname):
            data = load_probabilities(fname)
        else:
            dcos1, da1 = D['cos1'], D['a1']
            a1s = np.arange(0, 1, da1)
            ...
            for a1 in tqdm(a1s, desc="Building p_cos1 cache"):
                p_cos1_for_a1 = Parallel(n_jobs=num_cores)(
                    delayed(get_p_cos1_given_xeff_q_a1)(cos1, a1, self.xeff, self.q, n) for cos1 in cos1s)
        ...
        return Interped(
            xx=cos1, yy=p_cos1,
            minimum=min(cos1), maximum=max(cos1), name="cos_tilt_1",
    ...
    def get_cos2_prior(self, given_cos1, given_a1):
        ...
        kwargs = dict(xeff=self.xeff, q=self.q, a1=given_a1, cos1=given_cos1)
        p_c2 = np.array(
            [get_p_cos2_given_xeff_q_a1_cos1(cos2=cos2, **kwargs)
             for cos2 in cos2s]
        )
        p_c2 = p_c2 / np.sum(p_c2) / dc2
        return Interped(
            xx=cos2s, yy=p_c2,
            minimum=min(cos2s), maximum=max(cos2s), name="cos_tilt_2",
